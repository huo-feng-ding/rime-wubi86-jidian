...
local tags = { "uppercase" }

local function check_tag(context)
	local composition = context.composition
	if not composition:empty() then
		local segment = composition:back()
		--for a,tag in pairs(segment.tags) do
		--	log.info('tag', a, tag)
		--end
...
local function processor(key, env)
	...
	--log.info(repr, key.keycode, input, env.trigger_key, check_tag(context))

	if check_tag(context) and repr == "Return" then
		-- 如果是以分号开头的，则剔除开头的分号上屏
		if string.sub(input, 1, 1) == ";" then
			--log.info('commit', repr, input, env.trigger_key, check_tag(context))
